...
2. **Mining Engine:** Áp dụng thuật toán Apriori và FP-Growth để khai phá frequent itemsets và luật kết hợp. Hỗ trợ **tự động khai phá khi khởi động** (Auto-Mining) nếu chưa có luật trong cơ sở dữ liệu, đồng thời **tự động sinh combo** từ các luật kết hợp mạnh nhất.
...
| **Frontend** | React 18.2, Vite 5.4, React Router 6 | Single Page Application (SPA) |
| **Styling (CSS)** | Vanilla CSS | Giao diện kính mờ (Glassmorphism), vi hoạt ảnh |
...
graph LR
    subgraph "Client Layer (React SPA)"
        P1[Giao diện Public<br>10 trang]
        P2[Giao diện Admin<br>21 trang]
    end
...
│   ├── 06_dac_ta_chuc_nang.md         # Đặc tả chi tiết 33 chức năng
│   ├── 07_api_reference.md            # Tài liệu API endpoints
│   └── 08_bao_cao_do_an.md            # Báo cáo đồ án tổng hợp
│
...
│   ├── data/
│   │   └── raw/
│   │       └── hotel_bookings.csv      # Bộ dữ liệu gốc (119,390 dòng)
...
└── 📁 frontend/                        # ⚛️ React SPA Frontend (Vite)
    ...
    │   ├── pages/                      # 33 trang giao diện React
    │   ├── services/                   # Gọi API backend (Axios/Fetch)
    │   ├── styles/                     # Vanilla CSS thiết kế giao diện
...
Hệ thống TravelMind gồm **33 tính năng** được phân thành 3 nhóm chính:

### 7.1. 🌐 Trang công khai (10 trang React)
- **Trang chủ / Landing Page:** Giới thiệu TravelMind, banner AI, combo hot, xu hướng đặt phòng.
- **Danh sách khách sạn / Hotel List:** Duyệt danh sách khách sạn có filter theo loại, khu vực và sắp xếp.
- **Chi tiết khách sạn / Hotel Detail:** Xem chi tiết phòng, giá, tiện ích, ảnh và review của một khách sạn cụ thể.
...
### 7.2. 🔒 Trang quản trị (21 trang React)
...
- **Rules Lab:** Điều chỉnh min_support, min_confidence, chạy thuật toán Apriori/FP-Growth và xem biểu đồ phân bố luật kết hợp (Scatter Plot — Support vs Confidence vs Lift). Hỗ trợ tự động sinh combo từ luật.
- **AI Insights:** Phân tích và diễn giải luật kết hợp bằng AI, đề xuất chiến lược kinh doanh.
...
- **Quản lý đặt phòng / Booking Manager:** Duyệt, tìm kiếm và quản lý đơn đặt phòng của khách hàng.
- **Quản lý khách sạn / Hotel Manager:** CRUD khách sạn và phòng trong hệ thống.
...
- **Lịch sử nội dung / Content History:** Duyệt lịch sử toàn bộ nội dung AI đã sinh, theo dõi trạng thái kiểm duyệt.
- **Thư viện media / Media Library:** Quản lý toàn bộ media (ảnh, video) đã tải lên và sinh bởi AI.
- **Quản lý Template / Template Manager:** Quản lý các template prompt cho AI Content Studio.
...
> [!TIP]
> Khi server khởi động lần đầu, hệ thống sẽ **tự động chạy khai phá luật kết hợp (Auto-Mining)** nếu chưa có rules trong DB, và tự động tạo combo từ các luật mạnh nhất.
...
Sau khi chạy `seed_admin.py`, hệ thống sẽ tạo tài khoản admin mặc định:
