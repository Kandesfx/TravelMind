...
export default function UserProfile() {
  ...
  if (loading) {
    return <div style={{ color: 'var(--text-primary)', textAlign: 'center', marginTop: '5rem' }}>Đang tải hồ sơ cá nhân...</div>;
  }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '2.5rem' }} className="animate-fade-in">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <GlassCard style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem', textAlign: 'center' }} hover={false}>
          ...
          <div>
            <h3 style={{ fontSize: '1.3rem', color: 'var(--text-primary)' }}>{auth.user?.full_name || auth.user?.username}</h3>
            <span style={{ fontSize: '0.75rem', background: 'var(--hover-overlay)', padding: '0.2rem 0.5rem', borderRadius: '10px', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '0.25rem', display: 'inline-block' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: 'var(--text-primary)' }}>{auth.user?.email}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: 'var(--text-muted)' }}>Tham gia:</span>
              <span style={{ color: 'var(--text-primary)' }}>{auth.user?.created_at?.split('T')[0] || '2026-05-23'}</span>
            </div>
          </div>
        </GlassCard>
        ...
        <GlassCard style={{ display: 'flex', gap: '1rem', alignItems: 'center' }} hover={false}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
            <h4 style={{ color: 'var(--text-primary)', fontSize: '0.95rem' }}>Thành Viên Thân Thiết</h4>
      ...
      <GlassCard style={{ padding: '2rem' }} hover={false}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--section-border)', paddingBottom: '0.75rem' }}>
          <ClipboardList size={20} color="var(--primary)" />
          <h2 style={{ fontSize: '1.35rem', color: 'var(--text-primary)' }}>Lịch Sử Đặt Phòng Cá Nhân</h2>
        </div>
            ...
            <div key={b.id} style={{
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <h4 style={{ color: 'var(--text-primary)', fontSize: '1.05rem', fontWeight: 650 }}>{b.hotel_type}</h4>
                  <span style={{
                    fontSize: '0.7rem',
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', marginTop: '0.15rem' }}>${b.total_price.toFixed(2)}</div>
              </div>
            </div>
          ))}
        </div>
