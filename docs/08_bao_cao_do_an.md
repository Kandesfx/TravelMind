...
│   33 trang giao diện       │         │   REST API trả JSON      │
└────────────────────────────┘         └───────────────────────────┘
                                               ↓
                                       ┌───────────────────┐
                                       │   CƠ SỞ DỮ LIỆU  │
...
## 4.3. Các trang giao diện (33 trang)

### Trang công khai — Dành cho khách du lịch (10 trang)

| # | Trang | Chức năng chính |
|---|-------|-----------------|
| 1 | 🏠 **Trang chủ** | Giới thiệu TravelMind, combo hot nhất, xu hướng đặt phòng, nút bắt đầu khám phá |
| 2 | 🏨 **Danh sách Khách sạn** | Duyệt danh sách và filter khách sạn theo loại, khu vực |
| 3 | 🏨 **Chi tiết Khách sạn** | Xem chi tiết phòng, giá, tiện ích, ảnh và review của một khách sạn |
| 4 | 🏨 **Khám phá Khách sạn** | So sánh trực quan Resort vs City: thống kê, biểu đồ radar, combo phổ biến theo từng loại |
| 5 | 🎁 **Gợi ý Combo** ⭐ | Trang cốt lõi — Người dùng chọn 4 tiêu chí → nhận gợi ý Top 3 combo phù hợp nhất |
| 6 | 🧩 **Quiz du khách** | 5 câu hỏi trắc nghiệm xác định kiểu du khách (gia đình, công tác, lãng mạn...) → combo tương ứng |
| 7 | 📋 **Đặt phòng** | Form đặt phòng hoàn chỉnh, gợi ý nâng cấp dịch vụ, áp mã giảm giá |
| 8 | 📊 **Khám phá dữ liệu** | Biểu đồ tương tác: booking theo tháng, quốc gia, giá phòng, loại khách (Plotly) |
| 9 | 🎉 **Chi tiết sự kiện** | Trang sự kiện khuyến mãi với banner, combo đính kèm |
| 10 | 👤 **Hồ sơ cá nhân** | Lịch sử đặt phòng, voucher đã nhận, kết quả quiz |

### Trang quản trị — Dành cho quản lý (21 trang)
...
| 4 | ⚙️ Khai phá | Rules Lab | Chọn thuật toán, điều chỉnh ngưỡng, chạy phân tích, xem Scatter plot luật kết hợp. Hỗ trợ tự động sinh combo từ luật |
| 5 | 🤖 AI | AI Insights | Phân tích và diễn giải luật kết hợp bằng AI, đề xuất chiến lược kinh doanh |
| 6-11 | 💼 Kinh doanh | Combo, Ưu đãi, Sự kiện, Banner, Voucher, Booking, Hotel | Tạo/sửa/xóa các gói combo, khuyến mãi, sự kiện, banner, mã giảm giá, đơn đặt phòng, khách sạn |
| 12-13 | 📈 Báo cáo | Hiệu suất, Khách hàng | Theo dõi tỷ lệ chuyển đổi combo, phân tích hành vi 5 nhóm khách |
| 14-17 | 🤖 AI | Text Studio, Image Studio, Video Studio, Kiểm duyệt | Dùng AI viết mô tả, tạo ảnh, ghép video; quy trình duyệt Nháp → Phê duyệt → Xuất bản |
| 18-19 | 📚 AI | Thư viện Media, Template Manager | Quản lý media AI và template prompt |
| 20-21 | ⚙️ Cài đặt | API Keys, AI Usage | Quản lý khóa API (mã hóa AES-256), theo dõi chi phí AI |
...
## 4.5. Tích hợp AI Đa Phương Tiện (Text, Image & Video)

Hệ thống tích hợp giải pháp AI đa phương tiện tiên tiến nhằm biến các quy luật khai phá khô khan thành các chiến dịch truyền thông hấp dẫn:

### 1. ✍️ Sinh nội dung tiếp thị (AI Text Studio)
- **Công nghệ:** Sử dụng mô hình **Google Gemini 2.0 Flash** (`gemini-2.0-flash`) thông qua REST API v1beta.
- **Tính năng:** Tự động điền dữ liệu của combo/sự kiện (tên dịch vụ, đối tượng, mùa, giá, % giảm) vào các Prompt Template tùy chỉnh.
- **Văn phong:** Sinh ra 3 phiên bản văn bản (Thân thiện, Chuyên nghiệp, Hào hứng) ở định dạng JSON để người dùng dễ dàng chọn, chỉnh sửa và phê duyệt.

### 2. 🖼️ Tạo ảnh banner quảng cáo (AI Image Studio)
- **Công nghệ:** Tích hợp mô hình sinh ảnh hàng đầu **Google Imagen 3** (`imagen-3.0-generate-002`) sử dụng chung API Key Gemini giúp tiết kiệm tối đa chi phí cấu hình (~0.04 USD/ảnh).
- **Cơ chế phòng vệ (Matplotlib Fallback):** Nếu không có API Key hoặc bị lỗi kết nối, hệ thống kích hoạt bộ sinh ảnh gradient thích ứng cục bộ thông qua thư viện `matplotlib`. Gradient màu được sinh động động dựa trên nội dung prompt (Hè/Biển: Cam-Tím san hô; Lãng mạn/Cặp đôi: Hồng-Đỏ hoàng hôn; Đô thị: Xanh dương navy; Sang trọng: Tím-Vàng hoàng gia) kết hợp lớp phủ kính mờ Branding của TravelMind.

### 3. 🎥 Dựng video slideshow tiếp thị (AI Video Studio)
- **Công nghệ:** Sử dụng bộ engine **FFmpeg** cục bộ (`bin/ffmpeg.exe`) được nhúng trực tiếp trong backend dự án để kết xuất video MP4 thực tế không tốn phí bản quyền.
- **Tính năng cao cấp:**
  - **Ken Burns Effect:** Mỗi ảnh phân cảnh được áp dụng hiệu ứng phóng to/thu nhỏ động (zoompan) xoay chiều tự động để tạo cảm giác chuyển động điện ảnh mượt mà.
  - **Crossfade Transition:** Chuyển cảnh giữa các phân ảnh sử dụng bộ lọc `xfade` của FFmpeg.
  - **Text Draw Subtitles:** Tự động vẽ (drawtext) phụ đề đẹp mắt với viền bóng (shadow) tương ứng từng phân cảnh dựa trên kịch bản tiếp thị.
  - **Nhạc nền sinh động:** Hỗ trợ lồng ghép các bản nhạc nền định dạng chất lượng cao phù hợp với chủ đề chiến dịch (Tropical, Chillhop, Guitar...).
  - **Resilient Fallback:** Nếu chuỗi hiệu ứng phức tạp hoặc FFmpeg gặp sự cố, hệ thống tự động co dãn về command đơn giản (Single image loop) hoặc MP4 nhúng Base64 siêu nhỏ để đảm bảo ứng dụng không bao giờ bị đơ hay crash.

### 4. 🔒 Bảo mật và Quản trị Tài nguyên AI
- **Mã hóa AES-256:** Toàn bộ API Key của Gemini/các nhà cung cấp được mã hóa bằng thuật toán đối xứng AES-256 trước khi lưu vào SQLite database, sử dụng khóa mã hóa môi trường an toàn.
- **Usage & Cost Logs:** Ghi lại chi tiết từng lượt sử dụng AI (thời gian phản hồi, số token/ảnh sinh ra, chi phí ước tính USD) giúp ban quản trị trực quan hóa hiệu suất kinh doanh qua biểu đồ tiêu thụ tài nguyên.

## 4.6. Cơ sở dữ liệu

Hệ thống sử dụng SQLite với **19 bảng** dữ liệu, được tổ chức thành 5 nhóm:
...
> **Kết luận Phần 4:** Ứng dụng TravelMind là một nền tảng web hoàn chỉnh với 33 trang giao diện, tách biệt Frontend/Backend, tích hợp AI, cơ sở dữ liệu 19 bảng, và giao diện hiện đại. Tính năng cốt lõi là gợi ý combo dựa trên quy luật khai phá từ dữ liệu thật.
...
| `05_database.md` | Thiết kế cơ sở dữ liệu (19 bảng) | ~10 trang |
| `06_dac_ta_chuc_nang.md` | Đặc tả 33 chức năng | ~12 trang |
| `07_api_reference.md` | Tài liệu API (tất cả endpoints) | ~8 trang |
| `08_bao_cao_do_an.md` | Báo cáo đồ án tổng hợp (file này) | ~15 trang |
...
| **⚛️ Frontend** | Thiết kế giao diện 33 trang, biểu đồ, responsive, hiệu ứng | React, CSS, Chart.js |
...
| **Thành viên D** | Frontend + Thiết kế | `frontend/src/pages/`, `frontend/src/styles/`, `docs/06_*.md` | Thiết kế và lập trình 33 trang giao diện, viết tài liệu chức năng. |
